...
The RPM spec file provided here is for Evrmore-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://evrmorecoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Evrmore git repository](https://github.com/EvrmoreOrg/Evrmore) but are not part of
...
Currently only `Source10` is used, for the example `evrmore.conf` file.
...
in the evrmore git repository. At present only an SVG version of the Evrmore
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the evrmore
...
needed when 0.12.1 is released, a proper fix is already in the Evrmore git
master branch.
...
idea to build Evrmore Core against a static version of that library compiled
...
The `evrmore.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `evrmore-server` package. At this point, most RPM based
...
The `evrmore-server` package should have SELinux support. How to properly do
...
Evrmore-Core against OpenSSL or use a patched version of Boost in the build
system.
...
4. I renamed the `evrmore` package that contains the Qt GUI to `evrmore-core` as
that appears to be how the general population refers to it, in contrast to
`evrmore-xt` or `evrmore-classic`. I wanted to make sure the general population
