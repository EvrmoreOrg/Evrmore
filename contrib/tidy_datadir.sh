...
# Copyright (c) 2017-2019 The Evrmore Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Evrmore database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Evrmore datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Evrmore datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Evrmore 0.7 datadir."
    ;;
  3)
    echo "Detected Evrmore pre-0.8 datadir."
    ;;
  4)
    echo "Detected Evrmore 0.8 datadir."
    ;;
esac
