...
The Evrmore Doxygen documentation can be generated by running `make docs` from the top-level 
...
Run with the -testnet option to use the test network, if you
...
Evrmore Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- ThreadSocketHandler : Sends/Receives data from peers on port 8820.
...
- ThreadRPCServer : Remote procedure call handler, listens on port 8819 for connections and services them.

- EvrmoreMiner : Generates EVRs (if wallet is enabled).
...
However, in open source software such as Evrmore Core, where everyone uses
...
pay attention to for reviewers of Evrmore Core code.

General Evrmore Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Evrmore Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Evrmore so that they can be integrated
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:evrmore/evrmore.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `evrmore-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `evrmore-cli` and the GUI debug console use this table to determine how to
