...
const std::string EvrmoreGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString EvrmoreGUI::DEFAULT_WALLET = "~Default";

/* Bit of a bodge, c++ really doesn't want you to predefine values
 * in only header files, so we do one-time value assignment here. */
std::array<CurrencyUnitDetails, 6> CurrencyUnits::CurrencyOptions = { {
    { "BTC",    "EVRBTC"  , 1,          8},
    { "mBTC",   "EVRBTC"  , 1000,       5},
    { "µBTC",   "EVRBTC"  , 1000000,    2},
    { "Satoshi","EVRBTC"  , 100000000,  0},
    { "USDT",   "EVRUSDT" , 1,          5},
    { "RVN",    "EVRRVN"  , 1,          8}    
} };

static bool ThreadSafeMessageBox(EvrmoreGUI *gui, const std::string& message, const std::string& caption, unsigned int style);

EvrmoreGUI::EvrmoreGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
...
void EvrmoreGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Evrmore address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and evrmore: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to EVR addresses"));
...
//    messagingAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_9));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Evrmore addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Evrmore addresses"));
    ...
    openAction->setStatusTip(tr("Open a evrmore: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Evrmore command-line options").arg(tr(PACKAGE_NAME)));
...
void EvrmoreGUI::createToolBars()
{
    if(walletFrame)
    {
        QSettings settings;
        ...
        if(IconsOnly) {
            labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/evrtext")));
        }
        else {
            labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/evrmoretext")));
        }
...
//        m_toolbar->addAction(votingAction);
        ...
        QVBoxLayout* evrmoreLabelLayout = new QVBoxLayout(toolbarWidget);
        evrmoreLabelLayout->addWidget(labelToolbar);
        evrmoreLabelLayout->addWidget(m_toolbar);
        evrmoreLabelLayout->setDirection(QBoxLayout::TopToBottom);
        evrmoreLabelLayout->addStretch(1);
        ...
        labelCurrentMarket->setText(tr("Evrmore Market Price"));
        ...
        comboEvrUnit = new QComboBox(headerWidget);
        QStringList list;
        for(int unitNum = 0; unitNum < CurrencyUnits::count(); unitNum++) {
            list.append(QString(CurrencyUnits::CurrencyOptions[unitNum].Header));
        }
        comboEvrUnit->addItems(list);
        comboEvrUnit->setFixedHeight(26);
        comboEvrUnit->setContentsMargins(5,0,0,0);
        comboEvrUnit->setStyleSheet(STRING_LABEL_COLOR);
        comboEvrUnit->setFont(currentMarketFont);

        labelVersionUpdate->setText("<a href=\"https://github.com/EvrmoreOrg/Evrmore/releases\">New Wallet Version Available</a>");
        ...
        priceLayout->addWidget(comboEvrUnit, 0 , Qt::AlignBottom| Qt::AlignLeft);
        ...
        connect(comboEvrUnit, SIGNAL(activated(int)), this, SLOT(currencySelectionChanged(int)));
        // Create the timer
        ...
        // Get the latest Evrmore release and let the user know if they are using the latest version
        // Network request code for the header widget
                       ...
                       if (key == "tag_name") {
                           ...
                           if (fNewSoftwareFound) {
                               ...
                               if (GetRandInt(2) == 1) {
                                   ...
                                   if (fRet) {
                                       QString link = "https://github.com/EvrmoreOrg/Evrmore/releases";
...
void EvrmoreGUI::updateIconsOnlyToolbar(bool IconsOnly)
{
    if(IconsOnly) {
        labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/evrtext")));
        m_toolbar->setMaximumWidth(65);
...
void EvrmoreGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool EvrmoreGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool EvrmoreGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void EvrmoreGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void EvrmoreGUI::setWalletActionsEnabled(bool enabled)
{
...
void EvrmoreGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void EvrmoreGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void EvrmoreGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void EvrmoreGUI::optionsClicked()
{
...
void EvrmoreGUI::showDebugWindow()
{
...
void EvrmoreGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void EvrmoreGUI::showWalletRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void EvrmoreGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void EvrmoreGUI::openClicked()
{
...
void EvrmoreGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void EvrmoreGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void EvrmoreGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void EvrmoreGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void EvrmoreGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void EvrmoreGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** RVN START */
void EvrmoreGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void EvrmoreGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void EvrmoreGUI::gotoManageAssetsPage()
{
    manageAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoManageAssetsPage();
};

void EvrmoreGUI::gotoRestrictedAssetsPage()
{
...
void EvrmoreGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Evrmore network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void EvrmoreGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void EvrmoreGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void EvrmoreGUI::updateHeadersSyncProgressLabel()
{
...
void EvrmoreGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void EvrmoreGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Evrmore"); // default title
...
void EvrmoreGUI::changeEvent(QEvent *e)
{
...
void EvrmoreGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void EvrmoreGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void EvrmoreGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "EVR")
        msg += tr("Amount: %1\n").arg(EvrmoreUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(EvrmoreUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void EvrmoreGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void EvrmoreGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool EvrmoreGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool EvrmoreGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void EvrmoreGUI::setHDStatus(int hdEnabled)
{
    QString icon = "";
...
void EvrmoreGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void EvrmoreGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void EvrmoreGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void EvrmoreGUI::detectShutdown()
{
...
void EvrmoreGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void EvrmoreGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void EvrmoreGUI::showModalOverlay()
{
...
static bool ThreadSafeMnemonic(EvrmoreGUI *gui, unsigned int style)
{
    bool modal = (style & CClientUIInterface::MODAL);
    // The SECURE flag has no effect in the Qt GUI.
    // bool secure = (style & CClientUIInterface::SECURE);
...
void EvrmoreGUI::subscribeToCoreSignals()
{
...
void EvrmoreGUI::unsubscribeFromCoreSignals()
{
...
void EvrmoreGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void EvrmoreGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<EvrmoreUnits::Unit> units = EvrmoreUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const EvrmoreUnits::Unit unit : units)
    {
    #ifndef QTversionPreFiveEleven
        max_width = qMax(max_width, fm.horizontalAdvance(EvrmoreUnits::name(unit)));
    #else
        max_width = qMax(max_width, fm.width(EvrmoreUnits::name(unit)));
    #endif
    }
...
void UnitDisplayStatusBarControl::createContextMenu(const PlatformStyle *platformStyle)
{
    ...
    {
        QAction *menuAction = new QAction(QString(EvrmoreUnits::name(u)), this);
...
{
    setText(EvrmoreUnits::name(newUnits));
}
...
void EvrmoreGUI::currencySelectionChanged(int unitIndex)
{
    if(clientModel && clientModel->getOptionsModel())
    {
...
void EvrmoreGUI::onCurrencyChange(int newIndex)
{
    qDebug() << "EvrmoreGUI::onPriceUnitChange: " + QString::number(newIndex);
    ...
    this->comboEvrUnit->setCurrentIndex(newIndex);
    this->getPriceInfo();
}

void EvrmoreGUI::getPriceInfo()
{
    request->setUrl(QUrl(QString("https://evrmorecoin.org/price-%1.html").arg(this->currentPriceDisplay->Ticker)));
    networkManager->get(*request);
}

#ifdef ENABLE_WALLET
void EvrmoreGUI::mnemonic()
{
...
void EvrmoreGUI::getLatestVersion()
{
    versionRequest->setUrl(QUrl("https://api.github.com/repos/EvrmoreOrg/Evrmore/releases"));
