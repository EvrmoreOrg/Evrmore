...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx)
{
    ...
    //coinbaseTx.vin[0].scriptSig = CScript() << CScriptNum(nHeight) << OP_0; // This causes problems for various 3r party software
    coinbaseTx.vin[0].scriptSig = CScript() << nHeight << OP_0;

    // EVR start minerdevfund
    const std::vector<CTxDestination> whitelisted =
        GetMinerDevFundWhitelist(chainparams.GetConsensus());
    if (!whitelisted.empty()) {
        CAmount minerdevfundamount = GetMinerDevFundAmount(coinbaseTx.vout[0].nValue);
        coinbaseTx.vout[0].nValue -= minerdevfundamount;
        coinbaseTx.vout.emplace_back(minerdevfundamount,
                                     GetScriptForDestination(whitelisted[0]));
    }
    // EVR end minerdevfund
...
void IncrementExtraNonce(CBlock* pblock, const CBlockIndex* pindexPrev, unsigned int& nExtraNonce)
{
    ...
    //txCoinbase.vin[0].scriptSig = (CScript() << CScriptNum(nHeight) << CScriptNum(nExtraNonce)) + COINBASE_FLAGS; // this causes trouble for various 3rd party sw
...
void static EvrmoreMiner(const CChainParams& chainparams)
{
    LogPrintf("EvrmoreMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("evrmore-miner");
    ...
    if (!EnsureWalletIsAvailable(pWallet, false)) {
        LogPrintf("EvrmoreMiner -- Wallet not available\n");
    }
#endif
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("EvrmoreMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("EvrmoreMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("EvrmoreMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("EvrmoreMiner -- runtime error: %s\n", e.what());
        return;
    }
}

int GenerateEVRs(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++){
        minerThreads->create_thread(boost::bind(&EvrmoreMiner, boost::cref(chainparams)));
    }
