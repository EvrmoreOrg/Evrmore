...
BOOST_FIXTURE_TEST_SUITE(multisig_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(multisig_isstandard_test)
    {
        ...
        a_and_b << OP_2 << ToByteVector(key[0].GetPubKey()) << ToByteVector(key[1].GetPubKey()) << OP_2
                << OP_CHECKMULTISIG;
        BOOST_CHECK(::IsStandard(a_and_b, whichType, scriptType));

        CScript a_or_b;
        a_or_b << OP_1 << ToByteVector(key[0].GetPubKey()) << ToByteVector(key[1].GetPubKey()) << OP_2
               << OP_CHECKMULTISIG;
        BOOST_CHECK(::IsStandard(a_or_b, whichType, scriptType));

        CScript escrow;
        escrow << OP_2 << ToByteVector(key[0].GetPubKey()) << ToByteVector(key[1].GetPubKey())
               << ToByteVector(key[2].GetPubKey()) << OP_3 << OP_CHECKMULTISIG;
        BOOST_CHECK(::IsStandard(escrow, whichType, scriptType));
        ...
        BOOST_CHECK(!::IsStandard(one_of_four, whichType, scriptType));
        ...
        for (int i = 0; i < 6; i++)
            BOOST_CHECK(!::IsStandard(malformed[i], whichType, scriptType));
    }
