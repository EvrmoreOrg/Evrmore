...
cd ${GITHUB_WORKSPACE}
PKGVERSION=`grep "PACKAGE_VERSION" src/config/evrmore-config.h | cut -d\" -f2`
VERSION="${PKGVERSION}"
...
if [[ ${GITHUB_BASE_REF} =~ "release" ]]; then
    DISTNAME="evrmore-${VERSION}"
else
    DISTNAME="evrmore-${VERSION}-${SHORTHASH}"
fi
...
if [[ ${OS} == "windows" ]]; then
    ...
    for rmfile in detach-sig-create.sh win-codesign.cert evrmore-cli.exe evrmore-qt.exe evrmored.exe; do
        if [[ -e ${rmfile} ]]; then
            rm -f ${rmfile}
        fi
    done
    ...
    if [[ -e ${GITHUB_WORKSPACE}/dist/Evrmore-Qt.app/Contents/MacOS/install_cli.sh ]]; then
        chmod +x ${GITHUB_WORKSPACE}/dist/Evrmore-Qt.app/Contents/MacOS/install_cli.sh
    fi
    ...
    ${GITHUB_WORKSPACE}/depends/x86_64-apple-darwin14/native/bin/dmg dmg "Evrmore-Core.dmg" ${RELEASE_LOCATION}/${DISTNAME}-osx-unsigned.dmg
    ...
    if [[ -e ${RELEASE_LOCATION} ]]; then
        ...
        cd ${STAGE_DIR}
        cp -Rf ${DISTNAME}/bin/evrmored .
        cp -Rf ${DISTNAME}/bin/evrmore-cli .
    else
        echo "release directory doesn't exist"
    fi
