...
dnl Output: If qt version is auto, set evrmore_enable_qt to false. Else, exit.
AC_DEFUN([EVRMORE_QT_FAIL],[
  if test "x$evrmore_qt_want_version" = xauto && test "x$evrmore_qt_force" != xyes; then
    if test "x$evrmore_enable_qt" != xno; then
      AC_MSG_WARN([$1; evrmore-qt frontend will not be built])
    fi
    evrmore_enable_qt=no
    evrmore_enable_qt_test=no
  else
    AC_MSG_ERROR([$1])
  fi
])

AC_DEFUN([EVRMORE_QT_CHECK],[
  if test "x$evrmore_enable_qt" != xno && test "x$evrmore_qt_want_version" != xno; then
    true
    $1
  else
    true
...
dnl EVRMORE_QT_PATH_PROGS([FOO], [foo foo2], [/path/to/search/first], [continue if missing])
...
AC_DEFUN([EVRMORE_QT_PATH_PROGS],[
  EVRMORE_QT_CHECK([
    if test "x$3" != x; then
      AC_PATH_PROGS($1,$2,,$3)
    else
    ...
    if test "x$$1" = x && test "x$4" != xyes; then
      EVRMORE_QT_FAIL([$1 not found])
    fi
  ])
])

dnl Initialize qt input.
dnl This must be called before any other EVRMORE_QT* macros to ensure that
dnl input variables are set correctly.
dnl CAUTION: Do not use this inside of a conditional.
AC_DEFUN([EVRMORE_QT_INIT],[
  dnl enable qt support
  AC_ARG_WITH([gui],
    [AS_HELP_STRING([--with-gui@<:@=no|qt5|auto@:>@],
    [build evrmore-qt GUI (default=auto)])],
    [
     evrmore_qt_want_version=$withval
     if test "x$evrmore_qt_want_version" = xyes; then
       evrmore_qt_force=yes
       evrmore_qt_want_version=auto
     fi
    ],
    [evrmore_qt_want_version=auto])
  ...
  AS_IF([test "x$with_gui" = xqt5_debug],
        [AS_CASE([$host],
                 ...
                 [qt_lib_suffix= ]); evrmore_qt_want_version=qt5],
        [qt_lib_suffix= ])
...
dnl
dnl   EVRMORE_QT_CONFIGURE([MINIMUM-VERSION])
dnl
dnl Outputs: See _EVRMORE_QT_FIND_LIBS
dnl Outputs: Sets variables for all qt-related tools.
dnl Outputs: evrmore_enable_qt, evrmore_enable_qt_dbus, evrmore_enable_qt_test
AC_DEFUN([EVRMORE_QT_CONFIGURE],[
  qt_version=">= $1"
  qt_lib_prefix="Qt5"
  EVRMORE_QT_CHECK([_EVRMORE_QT_FIND_LIBS])

  dnl This is ugly and complicated. Yuck. Works as follows:
  dnl For Qt5, we can check a header to find out whether Qt is build
  dnl statically. When Qt is built statically, some plugins must be linked into
  dnl the final binary as well.
  dnl _EVRMORE_QT_CHECK_STATIC_PLUGIN does a quick link-check and appends the
  ...
  _EVRMORE_QT_IS_STATIC
  if test "x$evrmore_cv_static_qt" = xyes; then
    _EVRMORE_QT_CHECK_STATIC_LIBS
    ...
    if test "x$TARGET_OS" != xandroid; then
      _EVRMORE_QT_CHECK_STATIC_PLUGIN([QMinimalIntegrationPlugin], [-lqminimal])
    ...
    if test "x$TARGET_OS" = xwindows; then
      ...
      _EVRMORE_QT_CHECK_STATIC_PLUGIN([QWindowsIntegrationPlugin], [-lqwindows])
      ...
      _EVRMORE_QT_CHECK_STATIC_PLUGIN([QXcbIntegrationPlugin], [-lqxcb])
      ...
      _EVRMORE_QT_CHECK_STATIC_PLUGIN([QCocoaIntegrationPlugin], [-lqcocoa])
      _EVRMORE_QT_CHECK_STATIC_PLUGIN([QMacStylePlugin], [-lqmacstyle])
  ...
  EVRMORE_QT_PATH_PROGS([MOC], [moc-qt5 moc5 moc], $qt_bin_path)
  EVRMORE_QT_PATH_PROGS([UIC], [uic-qt5 uic5 uic], $qt_bin_path)
  EVRMORE_QT_PATH_PROGS([RCC], [rcc-qt5 rcc5 rcc], $qt_bin_path)
  EVRMORE_QT_PATH_PROGS([LRELEASE], [lrelease-qt5 lrelease5 lrelease], $qt_bin_path)
  EVRMORE_QT_PATH_PROGS([LUPDATE], [lupdate-qt5 lupdate5 lupdate],$qt_bin_path, yes)
  ...
  EVRMORE_QT_CHECK([
    evrmore_enable_qt=yes
    evrmore_enable_qt_test=yes
    if test "x$have_qt_test" = xno; then
      evrmore_enable_qt_test=no
    fi
    evrmore_enable_qt_dbus=no
    if test "x$use_dbus" != xno && test "x$have_qt_dbus" = xyes; then
      evrmore_enable_qt_dbus=yes
    fi
  ...
  if test x$evrmore_enable_qt = xyes; then
    AC_MSG_RESULT([$evrmore_enable_qt ($qt_lib_prefix)])
  else
    AC_MSG_RESULT([$evrmore_enable_qt])
  fi
...
dnl Output: evrmore_cv_static_qt=yes|no
AC_DEFUN([_EVRMORE_QT_IS_STATIC],[
  AC_CACHE_CHECK(for static Qt, evrmore_cv_static_qt,[
    AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
        #include <QtCore/qconfig.h>
        #ifndef QT_VERSION
        #  include <QtCore/qglobal.h>
      ...
      [evrmore_cv_static_qt=yes],
      [evrmore_cv_static_qt=no])
    ])
])

dnl Internal. Check if the link-requirements for a static plugin are met.
dnl
dnl _EVRMORE_QT_CHECK_STATIC_PLUGIN(PLUGIN, LIBRARIES)
dnl --------------------------------------------------
dnl
dnl Requires: INCLUDES and LIBS must be populated as necessary.
...
AC_DEFUN([_EVRMORE_QT_CHECK_STATIC_PLUGIN], [
  ...
  AC_LINK_IFELSE([AC_LANG_PROGRAM([[
    ...
    [AC_MSG_RESULT([no]); EVRMORE_QT_FAIL([$1 not found.])])
  LIBS="$CHECK_STATIC_PLUGINS_TEMP_LIBS"
])

dnl Internal. Check Qt static libs with PKG_CHECK_MODULES.
dnl
dnl _EVRMORE_QT_CHECK_STATIC_LIBS
dnl -----------------------------
dnl
dnl Inputs: no inputs.
dnl Outputs: QT_LIBS is prepended.
AC_DEFUN([_EVRMORE_QT_CHECK_STATIC_LIBS], [
...
AC_DEFUN([_EVRMORE_QT_FIND_LIBS],[
  EVRMORE_QT_CHECK([
    PKG_CHECK_MODULES([QT_CORE], [${qt_lib_prefix}Core${qt_lib_suffix} $qt_version], [],
                      [EVRMORE_QT_FAIL([${qt_lib_prefix}Core${qt_lib_suffix} $qt_version not found])])
  ])
  EVRMORE_QT_CHECK([
    PKG_CHECK_MODULES([QT_GUI], [${qt_lib_prefix}Gui${qt_lib_suffix} $qt_version], [],
                      [EVRMORE_QT_FAIL([${qt_lib_prefix}Gui${qt_lib_suffix} $qt_version not found])])
  ])
  EVRMORE_QT_CHECK([
    PKG_CHECK_MODULES([QT_WIDGETS], [${qt_lib_prefix}Widgets${qt_lib_suffix} $qt_version], [],
                      [EVRMORE_QT_FAIL([${qt_lib_prefix}Widgets${qt_lib_suffix} $qt_version not found])])
  ])
  EVRMORE_QT_CHECK([
    PKG_CHECK_MODULES([QT_NETWORK], [${qt_lib_prefix}Network${qt_lib_suffix} $qt_version], [],
                      [EVRMORE_QT_FAIL([${qt_lib_prefix}Network${qt_lib_suffix} $qt_version not found])])
  ])
