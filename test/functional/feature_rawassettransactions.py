...
class RawAssetTransactionsTest(EvrmoreTestFramework):
    ...
    def reissue_tampering_test(self):
        ...
        for n in range(0, len(tx.vout)):
            out = tx.vout[n]
            if evrr in bytes_to_hex_str(out.scriptPubKey):
                script_hex = bytes_to_hex_str(out.scriptPubKey)
                reissue_script_hex = script_hex[script_hex.index(evrr) + len(evrr):-len(op_drop)]
                ...
                tampered_script = script_hex[:script_hex.index(evrr)] + evrr + tampered_reissue + op_drop
        ...
        bad_vout = list(filter(lambda out_script: evrt not in bytes_to_hex_str(out_script.scriptPubKey), tx.vout))
        tx.vout = bad_vout
        tx_hex_bad = bytes_to_hex_str(tx.serialize())
        tx_signed = n0.signrawtransaction(tx_hex_bad)['hex']
    ...
    def issue_tampering_test(self):
        ...
        evro = '6576726f'  # evro
        # remove the owner output from vout
        bad_vout = list(filter(lambda out_script: evro not in bytes_to_hex_str(out_script.scriptPubKey), tx.vout))
        tx.vout = bad_vout
        ...
        owner_vout = list(filter(lambda out_script: evro in bytes_to_hex_str(out_script.scriptPubKey), tx.vout))[0]
        ...
        evrq = '65767271'  # evrq
        # remove the owner output from vout
        bad_vout = list(filter(lambda out_script: evrq not in bytes_to_hex_str(out_script.scriptPubKey), tx.vout))
        tx.vout = bad_vout
        tx_bad_issue = bytes_to_hex_str(tx.serialize())
        tx_bad_issue_signed = n0.signrawtransaction(tx_bad_issue)['hex']
        assert_raises_rpc_error(-26, "bad-txns-bad-asset-transaction",
        ...
        for n in range(0, len(tx.vout)):
            out = tx.vout[n]
            if evro in bytes_to_hex_str(out.scriptPubKey):
                owner_out = out
                owner_script_hex = bytes_to_hex_str(owner_out.scriptPubKey)
                asset_script_hex = owner_script_hex[owner_script_hex.index(evro) + len(evro):-len(op_drop)]
                f = BytesIO(hex_str_to_bytes(asset_script_hex))
                owner = CScriptOwner()
                owner.deserialize(f)
                owner.name = b"NOT_MY_ASSET!"
                tampered_owner = bytes_to_hex_str(owner.serialize())
                tampered_script = owner_script_hex[:owner_script_hex.index(evro)] + evro + tampered_owner + op_drop
                tx.vout[n].scriptPubKey = hex_str_to_bytes(tampered_script)
        tx_bad_issue = bytes_to_hex_str(tx.serialize())
        tx_bad_issue_signed = n0.signrawtransaction(tx_bad_issue)['hex']
        ...
        for n in range(0, len(tx.vout)):
            ...
            if evro in bytes_to_hex_str(out.scriptPubKey):
                ...
                tampered_script = owner_script_hex.replace(evro, EVRO)
                tx.vout[n].scriptPubKey = hex_str_to_bytes(tampered_script)
        tx_bad_issue = bytes_to_hex_str(tx.serialize())
        tx_bad_issue_signed = n0.signrawtransaction(tx_bad_issue)['hex']
        assert_raises_rpc_error(-26, "bad-txns-op-evr-asset-not-in-right-script-location",
                                n0.sendrawtransaction, tx_bad_issue_signed)
    ...
    def issue_reissue_transfer_test(self):
        ...
        for i in range(1, 3):
            ...
            f = BytesIO(hex_str_to_bytes(script_hex[script_hex.index(evrt) + len(evrt):-len(op_drop)]))
            ...
            tampered_script = script_hex[:script_hex.index(evrt)] + evrt + tampered_transfer + op_drop
        ...
        for n in range(0, len(tx.vout)):
            out = tx.vout[n]
            if evrt in bytes_to_hex_str(out.scriptPubKey):
                script_hex = bytes_to_hex_str(out.scriptPubKey)
                f = BytesIO(hex_str_to_bytes(script_hex[script_hex.index(evrt) + len(evrt):-len(op_drop)]))
                transfer = CScriptTransfer()
                transfer.deserialize(f)
    ...
    def issue_sub_multiple_outputs_test(self):
        ...
        for n in range(0, len(tx.vout)):
            ...
            if evrt in bytes_to_hex_str(out.scriptPubKey):
                ...
                asset_script_hex = transfer_script_hex[transfer_script_hex.index(evrt) + len(evrt):-len(op_drop)]
                f = BytesIO(hex_str_to_bytes(asset_script_hex))
                transfer = CScriptTransfer()
                transfer.deserialize(f)
                transfer.amount = 0
                tampered_transfer = bytes_to_hex_str(transfer.serialize())
                tampered_script = transfer_script_hex[:transfer_script_hex.index(evrt)] + evrt + tampered_transfer + op_drop
                tx.vout[n].scriptPubKey = hex_str_to_bytes(tampered_script)
    ...
    def transfer_asset_inserting_tampering_test(self):
        ...
        for n in range(0, len(tx.vout)):
            ...
            if evrt in bytes_to_hex_str(out.scriptPubKey):
                ...
                # Generate a script that has a valid destination address but switch it with evrq and the issue_serialized data
                issue_out.scriptPubKey = hex_str_to_bytes(transfer_script_hex[:transfer_script_hex.index(evrt)] + evrq + issue_serialized + op_drop)
    ...
    def atomic_swaps_test(self):
        ...
        # n1 buys 400 ANDUIN from n2 for 4000 EVR
        price = 4000
        amount = 400
        fee = 0.01
        ...
        # n2 buys JAINA! from n1 for 20000 EVR
        price = 20000
        amount = 1
    ...
    def fundrawtransaction_transfer_outs(self):
        ...
        for _ in range(0, 5):
            n0.sendtoaddress(n2_address, evr_amount / 5)
        n0.generate(1)
        self.sync_all()
